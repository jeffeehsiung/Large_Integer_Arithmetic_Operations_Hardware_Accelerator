...
module uart_top_sub #(
  ...
  localparam s_WAIT_RS      = 3'b011; //3
  localparam s_MP_ADD       = 3'b100; //4
 ...
 mp_adder_sub #( .OPERAND_WIDTH(OPERAND_WIDTH), .ADDER_WIDTH(ADDER_WIDTH), .N_ITERATIONS(OPERAND_WIDTH / ADDER_WIDTH))
 MP_ADDER_INST
     (.iClk(iClk),
     .iRst(iRst),
     .iStart(rStart),
  ...
  if (iRst == 1 ) 
    ...
    begin
      case (rFSM)
        ...
        s_WAIT_RB :
          begin    
            if(wRxDone) begin
                ...
                if (rCnt == NBYTES) begin                             // if this is the n-th byte, store the byte                                                                      
                    rCnt <= 0;                                        // reset counter to 0
                    rFSM <= s_WAIT_RS;                                 // rB also has been filled, go to s_WAIT_RS
                end                                     
            end else begin 
        ...
        // TODO: integrate rStart that will be set to high when 2x 64 + 1 bytes are received.
        s_WAIT_RS :
          begin    
            if(wRxDone) begin
                if( rCnt < 1) begin
                    rSub <= wRxByte;                                  // we receive and store in one subtract-or-not byte                                                                                                                                
                    rCnt <= rCnt+1;                                   // increment the counter
                    rFSM <= s_WAIT_RS;                                 // rS also has been filled, go to s_MP_ADD 
                end 
                else begin                                                                    
                    rCnt <= 0;                                        // reset counter to 0
                    rStart <= 1;                                      // set rStart high for next state
                    rFSM <= s_MP_ADD;                                 // rB also has been filled, go to s_WAIT_RS
                end                          
            end else begin 
                rFSM <= s_WAIT_RS;                                    // rx still busy, stay in this state
                if (rCnt == 1) begin                             // if this is the n-th byte, store the byte                                                                        
                    rCnt <= 0;                                        // reset counter to 0
                    rStart <= 1;                                      // set rStart high for next state
                    rFSM <= s_MP_ADD;                                 // rB also has been filled, go to s_WAIT_RS
                end 
